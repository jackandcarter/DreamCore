...
const DEFAULT_WEAPON_BRANCH = 'retail';
const WEAPON_BRANCH_UI = {
  retail: {
    key: 'retail',
    label: `${CONFIG.BRAND_NAME || 'DreamCore Master'} (Retail)`,
    description: 'Retail (master) hotfix tables',
  },
  classic: {
    key: 'classic',
    label: `${CONFIG.CLASSIC_BRAND_NAME || 'DreamCore Classic'} (3.3.5)`,
    description: 'Wrath 3.3.5 world tables',
  },
};
...
const HOTFIX_DB = {
  HOST: process.env.HOTFIX_DB_HOST || WORLD_DB.HOST,
  PORT: Number(process.env.HOTFIX_DB_PORT || WORLD_DB.PORT || 3306),
  USER: process.env.HOTFIX_DB_USER || WORLD_DB.USER || 'trinity',
  PASS: process.env.HOTFIX_DB_PASS || WORLD_DB.PASS || DEFAULT_DB_PASS,
  NAME: process.env.HOTFIX_DB_NAME || 'hotfixes',
};
...
const hotfixPool =
  isSameDbConfig(WORLD_DB, HOTFIX_DB)
    ? worldPool
    : await mysql.createPool({
        host: HOTFIX_DB.HOST,
        port: HOTFIX_DB.PORT,
        user: HOTFIX_DB.USER,
        password: HOTFIX_DB.PASS,
        database: HOTFIX_DB.NAME,
        waitForConnections: true,
        connectionLimit: 10,
        namedPlaceholders: true,
      });
...
const requireGmRealmQuery = requireRequestGm({ source: 'query', key: 'realm', fallbackRealm: 'retail' });
const requireGmRealmBody = requireRequestGm({ source: 'body', key: 'realm', fallbackRealm: 'retail' });
const requireWeaponBranchQuery = requireRequestGm({
  source: 'query',
  key: 'branch',
  fallbackRealm: DEFAULT_WEAPON_BRANCH,
});
const requireWeaponBranchBody = requireRequestGm({
  source: 'body',
  key: 'branch',
  fallbackRealm: DEFAULT_WEAPON_BRANCH,
});
  ...
  <script>
    ...
    window.REGISTRATION_HELPER_COPY = ${JSON.stringify({
      ...
      classic: {
        ...
        body: 'Classic accounts are for the Wrath of the Lich King realm and original 3.3.5 client.',
...
function buildPortalLimitsScriptTag() {
  const payload = {
    ...
    weaponBranches: WEAPON_BRANCH_UI,
    defaultWeaponBranch: DEFAULT_WEAPON_BRANCH,
    ...
    gmWeaponSearchEndpoint: '/api/gm/weapons/search',
    gmWeaponDetailEndpoint: '/api/gm/weapons',
    gmWeaponCreateEndpoint: '/api/gm/weapons',
    gmQuestSearchEndpoint: '/api/gm/quests/search',
    gmQuestDetailEndpoint: '/api/gm/quests',
    gmQuestCreateEndpoint: '/api/gm/quests',
    gmActorSearchEndpoint: '/api/gm/actors/search',
    gmActorDetailEndpoint: '/api/gm/actors',
  };
  return `
  <script>
        ...
        <div class="mt-8 space-y-6">
          ...
          <section id="gmToolkitSection" data-tab-panel class="hidden rounded-3xl gradient-border bg-gray-900/70 p-6 shadow-inner shadow-violet-900/30">
            <div class="flex flex-col gap-6 lg:flex-row">
              <div class="flex-1">
                <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5 space-y-5">
                  <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p class="text-xs font-semibold uppercase tracking-[0.35em] text-indigo-300">GM toolkit</p>
                      <h2 class="text-2xl font-semibold text-white">Game master workflow suite</h2>
                      <p class="text-sm text-indigo-100/80">Jump between the live SOAP console, quest builder, and weapon maker without leaving this tab.</p>
                    </div>
                  ...
                  <div class="flex flex-wrap gap-2" role="tablist" aria-label="GM toolkit tools">
                    <button type="button" data-gmtool-tab="server" aria-controls="gmServerPanel" aria-selected="true" class="gm-tool-tab flex-1 min-w-[120px] rounded-2xl border border-white/10 bg-gray-900/60 px-4 py-2 text-sm font-semibold text-indigo-100 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Server</button>
                    <button type="button" data-gmtool-tab="weapon" aria-controls="gmWeaponPanel" aria-selected="false" class="gm-tool-tab flex-1 min-w-[120px] rounded-2xl border border-white/10 bg-gray-900/60 px-4 py-2 text-sm font-semibold text-indigo-100/70 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Weapon maker</button>
                  </div>
                  <div class="space-y-5">
                    <div data-gmtool-panel="server" id="gmServerPanel" class="space-y-5">
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            <h3 class="text-xl font-semibold text-white">SOAP command console</h3>
                            <p class="text-sm text-indigo-100/80">Send live commands to ${CONFIG.BRAND_NAME} and ${CONFIG.CLASSIC_BRAND_NAME} realms.</p>
                          </div>
                          <span class="rounded-full border border-white/10 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.35em] text-indigo-200/80">Live</span>
                        </div>
                      ...
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5">
                        <div class="flex items-center justify-between">
                          ...
                          <span class="text-xs font-medium uppercase tracking-[0.35em] text-indigo-200/80">Session</span>
                        </div>
                      ...
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.35em] text-indigo-300">Quest maker</p>
                            <h3 class="text-lg font-semibold text-white">Build a new quest chain</h3>
                            <p class="text-sm text-indigo-100/80">Clone quest data, edit texts, and attach it to a brand new NPC or game object.</p>
                          </div>
                          <span class="text-xs font-semibold uppercase tracking-[0.35em] text-indigo-200/80">Beta</span>
                        </div>
                        <form id="questSearchForm" class="mt-4 flex flex-col gap-3 sm:flex-row">
                          <div class="flex-1">
                            <label class="block text-sm font-medium text-indigo-200 mb-1" for="questSearchInput">Search quests</label>
                            <input id="questSearchInput" type="text" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="Wrathgate, 13124" />
                          </div>
                          <div class="flex gap-2 sm:flex-col">
                            <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-900/40 transition hover:from-indigo-400 hover:via-purple-400 hover:to-blue-400 focus:ring-2 focus:ring-indigo-400">Search</button>
                            <button type="button" id="questSearchReset" class="inline-flex items-center justify-center rounded-2xl border border-indigo-400/60 px-4 py-2 text-sm font-semibold text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Clear</button>
                          </div>
                        </form>
                        <p id="questSearchStatus" class="mt-2 text-sm text-indigo-200/80">Search for a quest to clone or reuse as a starting point.</p>
                        <div id="questSearchResults" class="mt-4 max-h-64 space-y-3 overflow-y-auto rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-100/85">
                          <p class="text-sm text-indigo-200/70">No search performed yet.</p>
                        </div>
                      </div>
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5" id="questEditorCard">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                          <div>
                            <h3 class="text-lg font-semibold text-white">Quest blueprint</h3>
                            <p id="questEditorStatus" class="text-sm text-indigo-200/80">Select a source quest to begin editing.</p>
                          </div>
                          <div class="flex flex-col gap-2 sm:flex-row sm:items-end">
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questNewEntryInput">New quest ID</label>
                              <input id="questNewEntryInput" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="100000" />
                            </div>
                            <button id="questResetButton" type="button" class="inline-flex items-center justify-center rounded-2xl border border-white/15 px-4 py-2 text-sm font-semibold text-indigo-100 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-white/40" disabled>Reset blueprint</button>
                          </div>
                        </div>
                        <div id="questFieldGrid" class="mt-5 space-y-4">
                          <div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to load a source quest.</div>
                        </div>
                        <div class="mt-5 grid gap-4 lg:grid-cols-2">
                          <div>
                            <label class="block text-sm font-medium text-indigo-200 mb-1" for="questAdvancedOverrides">Advanced overrides (JSON)</label>
                            <textarea id="questAdvancedOverrides" rows="4" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder='{"QuestFlags":33554432}'></textarea>
                            <p class="mt-1 text-xs text-indigo-200/70">Optional JSON overrides merged into quest_template.</p>
                          </div>
                          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end lg:flex-col lg:items-end">
                            <button id="questPreviewButton" type="button" class="inline-flex items-center justify-center rounded-2xl border border-indigo-400/60 bg-gray-900/60 px-5 py-2 text-sm font-semibold text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Preview quest</button>
                            <button id="questCreateButton" type="button" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-green-500 via-emerald-500 to-cyan-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-900/40 transition hover:from-green-400 hover:via-emerald-400 hover:to-cyan-400 focus:ring-2 focus:ring-emerald-400">Create quest + actor</button>
                          </div>
                        </div>
                        <pre id="questPreviewOutput" class="mt-4 rounded-2xl border border-white/5 bg-black/30 p-4 text-xs text-indigo-100/90 min-h-[120px] whitespace-pre-wrap">Waiting for preview data…</pre>
                      </div>
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5" id="questActorCard">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                          <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.35em] text-indigo-300">Quest placement</p>
                            <h3 class="text-lg font-semibold text-white">Quest giver builder</h3>
                            <p class="text-sm text-indigo-100/80">Clone an NPC or game object, spawn it, and automatically attach the quest.</p>
                          </div>
                          <label class="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.35em] text-indigo-100">
                            <input id="questActorToggle" type="checkbox" class="rounded border-white/20 bg-gray-900/60 text-indigo-500 focus:ring-indigo-400" checked />
                            Auto-create quest giver
                          </label>
                        </div>
                        <div id="questActorFields" class="mt-4 space-y-4">
                          <div class="grid gap-4 md:grid-cols-3">
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorTypeSelect">Actor type</label>
                              <select id="questActorTypeSelect" class="dark-select w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400">
                                <option value="creature">Creature</option>
                                <option value="gameobject">Game object</option>
                              </select>
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSourceInput">Source entry</label>
                              <input id="questActorSourceInput" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="Select from search" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorNewEntryInput">New entry ID</label>
                              <input id="questActorNewEntryInput" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="910000" />
                            </div>
                          </div>
                          <form id="questActorSearchForm" class="rounded-2xl border border-white/10 bg-black/20 p-4 space-y-3">
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5">
                        <div class="flex flex-wrap items-center justify-between gap-3">
                          <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.35em] text-indigo-300">Weapon maker</p>
                            <h3 class="text-lg font-semibold text-white">Clone and customize a weapon</h3>
                            <p class="text-sm text-indigo-100/80">Pick a branch, duplicate an existing weapon, and tweak every column before saving.</p>
                          </div>
                          <div class="w-full max-w-xs">
                            <label class="block text-sm font-medium text-indigo-200 mb-1" for="weaponBranchSelect">Branch</label>
                            <select id="weaponBranchSelect" class="dark-select w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400">
                              <option value="retail">Retail</option>
                              <option value="classic">Classic</option>
                            </select>
                          </div>
                        </div>
                        <form id="weaponSearchForm" class="mt-4 flex flex-col gap-3 sm:flex-row">
                          <div class="flex-1">
                            <label class="block text-sm font-medium text-indigo-200 mb-1" for="weaponSearchInput">Search weapons</label>
                            <input id="weaponSearchInput" type="text" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="Ashbringer, 19019" />
                          </div>
                          <div class="flex gap-2 sm:flex-col">
                            <button type="submit" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-900/40 transition hover:from-indigo-400 hover:via-purple-400 hover:to-blue-400 focus:ring-2 focus:ring-indigo-400">Search</button>
                            <button type="button" id="weaponSearchReset" class="inline-flex items-center justify-center rounded-2xl border border-indigo-400/60 px-4 py-2 text-sm font-semibold text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Clear</button>
                          </div>
                        </form>
                        <p id="weaponSearchStatus" class="mt-2 text-sm text-indigo-200/80">Select a branch above and search for a source weapon.</p>
                        <div id="weaponSearchResults" class="mt-4 max-h-64 space-y-3 overflow-y-auto rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-100/85">
                          <p class="text-sm text-indigo-200/70">No search performed yet.</p>
                        </div>
                      </div>
                      <div class="rounded-2xl border border-white/10 bg-gray-900/70 p-5" id="weaponEditorCard">
                        <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                          <div>
                            <h3 class="text-lg font-semibold text-white">Weapon blueprint</h3>
                            <p id="weaponEditorStatus" class="text-sm text-indigo-200/80">Select a source weapon to populate the editor.</p>
                          </div>
                          <div class="flex flex-col gap-2 sm:flex-row sm:items-end">
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="weaponNewEntryInput">New entry ID</label>
                              <input id="weaponNewEntryInput" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="900000" />
                            </div>
                            <button id="weaponResetButton" type="button" class="inline-flex items-center justify-center rounded-2xl border border-white/15 px-4 py-2 text-sm font-semibold text-indigo-100 transition hover:text-white focus:outline-none focus:ring-2 focus:ring-white/40" disabled>Reset blueprint</button>
                          </div>
                        </div>
                        <div id="weaponFieldGrid" class="mt-5 space-y-4">
                          <div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to load a source weapon.</div>
                        </div>
                        <div class="mt-5 grid gap-4 lg:grid-cols-2">
                          <div>
                            <label class="block text-sm font-medium text-indigo-200 mb-1" for="weaponAdvancedOverrides">Advanced overrides (JSON)</label>
                            <textarea id="weaponAdvancedOverrides" rows="4" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder='{"item_sparse":{"Display":"Custom"}}'></textarea>
                            <p class="mt-1 text-xs text-indigo-200/70">Optional JSON object keyed by table (item_sparse, item, etc.).</p>
                          </div>
                          <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end lg:flex-col lg:items-end">
                            <button id="weaponPreviewButton" type="button" class="inline-flex items-center justify-center rounded-2xl border border-indigo-400/60 bg-gray-900/60 px-5 py-2 text-sm font-semibold text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Preview changes</button>
                            <button id="weaponCreateButton" type="button" class="inline-flex items-center justify-center rounded-2xl bg-gradient-to-r from-green-500 via-emerald-500 to-cyan-500 px-5 py-2 text-sm font-semibold text-white shadow-lg shadow-emerald-900/40 transition hover:from-green-400 hover:via-emerald-400 hover:to-cyan-400 focus:ring-2 focus:ring-emerald-400">Clone weapon</button>
                          </div>
                        </div>
                        <pre id="weaponPreviewOutput" class="mt-4 rounded-2xl border border-white/5 bg-black/30 p-4 text-xs text-indigo-100/90 min-h-[120px] whitespace-pre-wrap">Waiting for preview data…</pre>
                      </div>
                    </div>
                            <div class="grid gap-3 md:grid-cols-2">
                              <div>
                                <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSearchInput">Search actors</label>
                                <input id="questActorSearchInput" type="text" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="Thrall, 4949" />
                              </div>
                              <div class="flex items-end">
                                <button type="submit" class="inline-flex w-full items-center justify-center rounded-2xl border border-indigo-400/60 bg-gray-900/60 px-4 py-2 text-sm font-semibold text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400">Search templates</button>
                              </div>
                            </div>
                            <div id="questActorSearchResults" class="max-h-40 space-y-2 overflow-y-auto rounded-2xl border border-white/5 bg-gray-900/60 p-3 text-sm text-indigo-100/85">
                              <p class="text-sm text-indigo-200/70">Use the search box to find NPCs or game objects.</p>
                            </div>
                          </form>
                          <p id="questActorStatus" class="text-sm text-indigo-200/80">Auto-creation enabled.</p>
                          <div id="questActorTemplateFields" class="space-y-4">
                            <div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Load a source actor to edit the template details.</div>
                          </div>
                          <div class="grid gap-4 md:grid-cols-3">
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnMap">Map ID</label>
                              <input id="questActorSpawnMap" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="1" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnZone">Zone ID</label>
                              <input id="questActorSpawnZone" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="0" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnArea">Area ID</label>
                              <input id="questActorSpawnArea" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="0" />
                            </div>
                          </div>
                          <div class="grid gap-4 md:grid-cols-4">
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnX">Pos X</label>
                              <input id="questActorSpawnX" type="number" step="0.01" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="0" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnY">Pos Y</label>
                              <input id="questActorSpawnY" type="number" step="0.01" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="0" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnZ">Pos Z</label>
                              <input id="questActorSpawnZ" type="number" step="0.01" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="0" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnO">Orientation</label>
                              <input id="questActorSpawnO" type="number" step="0.01" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="0" />
                            </div>
                          </div>
                          <div class="grid gap-4 md:grid-cols-2">
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnPhase">Phase mask</label>
                              <input id="questActorSpawnPhase" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="1" />
                            </div>
                            <div>
                              <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorSpawnMask">Spawn mask</label>
                              <input id="questActorSpawnMask" type="number" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder="1" />
                            </div>
                          </div>
                          <div class="grid gap-3 md:grid-cols-2">
                            <label class="inline-flex items-center gap-2 text-sm font-semibold text-indigo-100">
                              <input id="questActorStarterToggle" type="checkbox" class="rounded border-white/20 bg-gray-900/60 text-indigo-500 focus:ring-indigo-400" checked />
                              Flag as quest starter
                            </label>
                            <label class="inline-flex items-center gap-2 text-sm font-semibold text-indigo-100">
                              <input id="questActorEnderToggle" type="checkbox" class="rounded border-white/20 bg-gray-900/60 text-indigo-500 focus:ring-indigo-400" checked />
                              Flag as quest ender
                            </label>
                          </div>
                          <div>
                            <label class="block text-sm font-medium text-indigo-200 mb-1" for="questActorAdvancedOverrides">Actor advanced overrides (JSON)</label>
                            <textarea id="questActorAdvancedOverrides" rows="3" class="glow-input w-full rounded-2xl p-3 text-[15px] font-semibold focus:outline-none focus:ring-2 focus:ring-indigo-400" placeholder='{"npcflag":2,"unit_flags":0}'></textarea>
                          </div>
                        </div>
                      </div>
                    </div>
...
const accountScript = () => {
  ...
  const WEAPON_BRANCH_META = LIMITS.weaponBranches || {};
  const DEFAULT_WEAPON_BRANCH = LIMITS.defaultWeaponBranch || 'retail';
  ...
  const STORAGE_KEYS = {
    ...
    weaponBranch: 'dreamcore.dashboard.weaponBranch',
  };
  ...
  const gmToolTabButtons = document.querySelectorAll('[data-gmtool-tab]');
  const gmToolPanels = document.querySelectorAll('[data-gmtool-panel]');
  ...
  const weaponSearchReset = document.getElementById('weaponSearchReset');
  ...
  const weaponEditorStatus = document.getElementById('weaponEditorStatus');
  const weaponFieldGrid = document.getElementById('weaponFieldGrid');
  const weaponAdvancedOverrides = document.getElementById('weaponAdvancedOverrides');
  const weaponPreviewOutput = document.getElementById('weaponPreviewOutput');
  const weaponNewEntryInput = document.getElementById('weaponNewEntryInput');
  const weaponResetButton = document.getElementById('weaponResetButton');
  const weaponPreviewButton = document.getElementById('weaponPreviewButton');
  const weaponCreateButton = document.getElementById('weaponCreateButton');
  const weaponBranchSelect = document.getElementById('weaponBranchSelect');
  const questSearchForm = document.getElementById('questSearchForm');
  const questSearchInput = document.getElementById('questSearchInput');
  const questSearchReset = document.getElementById('questSearchReset');
  const questSearchResults = document.getElementById('questSearchResults');
  const questSearchStatus = document.getElementById('questSearchStatus');
  const questEditorStatus = document.getElementById('questEditorStatus');
  const questFieldGrid = document.getElementById('questFieldGrid');
  const questAdvancedOverrides = document.getElementById('questAdvancedOverrides');
  const questPreviewOutput = document.getElementById('questPreviewOutput');
  const questNewEntryInput = document.getElementById('questNewEntryInput');
  const questResetButton = document.getElementById('questResetButton');
  const questPreviewButton = document.getElementById('questPreviewButton');
  const questCreateButton = document.getElementById('questCreateButton');
  const questActorToggle = document.getElementById('questActorToggle');
  const questActorFieldsWrapper = document.getElementById('questActorFields');
  const questActorTypeSelect = document.getElementById('questActorTypeSelect');
  const questActorSourceInput = document.getElementById('questActorSourceInput');
  const questActorNewEntryInput = document.getElementById('questActorNewEntryInput');
  const questActorSearchForm = document.getElementById('questActorSearchForm');
  const questActorSearchInput = document.getElementById('questActorSearchInput');
  const questActorSearchResults = document.getElementById('questActorSearchResults');
  const questActorStatus = document.getElementById('questActorStatus');
  const questActorTemplateFields = document.getElementById('questActorTemplateFields');
  const questActorAdvancedOverrides = document.getElementById('questActorAdvancedOverrides');
  const questActorSpawnMap = document.getElementById('questActorSpawnMap');
  const questActorSpawnZone = document.getElementById('questActorSpawnZone');
  const questActorSpawnArea = document.getElementById('questActorSpawnArea');
  const questActorSpawnX = document.getElementById('questActorSpawnX');
  const questActorSpawnY = document.getElementById('questActorSpawnY');
  const questActorSpawnZ = document.getElementById('questActorSpawnZ');
  const questActorSpawnO = document.getElementById('questActorSpawnO');
  const questActorSpawnPhase = document.getElementById('questActorSpawnPhase');
  const questActorSpawnMask = document.getElementById('questActorSpawnMask');
  const questActorStarterToggle = document.getElementById('questActorStarterToggle');
  const questActorEnderToggle = document.getElementById('questActorEnderToggle');
  ...
  const gmWeaponSearchEndpoint = LIMITS.gmWeaponSearchEndpoint || '/api/gm/weapons/search';
  const gmWeaponDetailEndpoint = LIMITS.gmWeaponDetailEndpoint || '/api/gm/weapons';
  const gmWeaponCreateEndpoint = LIMITS.gmWeaponCreateEndpoint || '/api/gm/weapons';
  const gmQuestSearchEndpoint = LIMITS.gmQuestSearchEndpoint || '/api/gm/quests/search';
  const gmQuestDetailEndpoint = LIMITS.gmQuestDetailEndpoint || '/api/gm/quests';
  const gmQuestCreateEndpoint = LIMITS.gmQuestCreateEndpoint || '/api/gm/quests';
  const gmActorSearchEndpoint = LIMITS.gmActorSearchEndpoint || '/api/gm/actors/search';
  const gmActorDetailEndpoint = LIMITS.gmActorDetailEndpoint || '/api/gm/actors';
  ...
  let activeGmTool = 'server';
  const weaponState = createWeaponState();
  const questState = createQuestState();
  let weaponJsonEditors = new Map();
  let questJsonEditor = null;
  let questActorJsonEditor = null;
  questState.actor.type = questActorTypeSelect?.value === 'gameobject' ? 'gameobject' : 'creature';
  questState.actor.enabled = questActorToggle?.checked !== false;

  function createWeaponState() {
    return {
      branch: DEFAULT_WEAPON_BRANCH,
      searchResults: [],
      loading: false,
      summary: null,
      tables: [],
  ...
  const storedWeaponBranch = readStoredValue(STORAGE_KEYS.weaponBranch);
  if (storedWeaponBranch) {
    weaponState.branch = normalizeWeaponBranchInput(storedWeaponBranch) || DEFAULT_WEAPON_BRANCH;
  }
  ...
  populateWeaponBranchSelect();

  function getActiveGmRealm() {
    const value = gmRealmSelect?.value || '';
    return value || null;
  }
  ...
  function setWeaponStatus(message) {
    if (weaponEditorStatus) {
      weaponEditorStatus.textContent = message || '';
    }
  }

  function setQuestSearchStatus(message) {
    if (questSearchStatus) {
      questSearchStatus.textContent = message || '';
    }
  }

  function setQuestStatus(message) {
    if (questEditorStatus) {
      questEditorStatus.textContent = message || '';
    }
  }

  function setQuestActorStatus(message) {
    if (questActorStatus) {
      questActorStatus.textContent = message || '';
    }
  }

  function normalizeWeaponBranchInput(value) {
    const text = typeof value === 'string' ? value.trim().toLowerCase() : '';
    if (text === 'classic') return 'classic';
    return 'retail';
  }

  function weaponBranchLabel(branchKey) {
    return WEAPON_BRANCH_META[branchKey]?.label || (branchKey === 'classic' ? 'Classic' : 'Retail');
  }
  ...
  function setWeaponSubmitting(state) {
    toggleButtonState(weaponPreviewButton, state);
    toggleButtonState(weaponCreateButton, state);
  }

  function setQuestSubmitting(state) {
    toggleButtonState(questPreviewButton, state);
    toggleButtonState(questCreateButton, state);
  }

  function resetWeaponBlueprint(clearSearch = false) {
    weaponState.summary = null;
    weaponState.tables = [];
    weaponState.suggestedEntry = null;
    weaponJsonEditors = new Map();
    if (weaponFieldGrid) {
      weaponFieldGrid.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to load a source weapon.</div>';
    }
    if (weaponPreviewOutput) {
      weaponPreviewOutput.textContent = 'Waiting for preview data…';
    }
    if (weaponNewEntryInput) {
      weaponNewEntryInput.value = '';
    }
    if (weaponResetButton) {
      weaponResetButton.disabled = true;
    }
    if (clearSearch) {
      weaponState.searchResults = [];
      renderWeaponSearchResults();
    }
  }

  function resetQuestBlueprint(clearSearch = false) {
    questState.source = null;
    questState.snapshot = '';
    questState.suggestedEntry = null;
    questJsonEditor = null;
    if (questFieldGrid) {
      questFieldGrid.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to load a source quest.</div>';
    }
    if (questPreviewOutput) {
      questPreviewOutput.textContent = 'Waiting for preview data…';
    }
    if (questNewEntryInput) {
      questNewEntryInput.value = '';
    }
    if (questResetButton) {
      questResetButton.disabled = true;
    }
    if (clearSearch) {
      questState.searchResults = [];
      renderQuestSearchResults();
    }
  }

  function resetQuestActor(clearSearch = false) {
    questState.actor.source = null;
    questState.actor.snapshot = '';
    questState.actor.suggestedEntry = null;
    questActorJsonEditor = null;
    if (questActorTemplateFields) {
      questActorTemplateFields.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Load a source actor to edit the template details.</div>';
    }
    if (questActorSourceInput && clearSearch) {
      questActorSourceInput.value = '';
    }
    if (questActorNewEntryInput && clearSearch) {
      questActorNewEntryInput.value = '';
    }
    if (clearSearch) {
      questState.actor.searchResults = [];
      renderQuestActorSearchResults();
    }
  }

  function renderWeaponSearchResults() {
    if (!weaponSearchResults) return;
    const results = Array.isArray(weaponState.searchResults) ? weaponState.searchResults : [];
    if (!results.length) {
      weaponSearchResults.innerHTML =
        '<p class="text-sm text-indigo-200/70">No weapon matches yet. Try a different keyword.</p>';
      return;
    }
    weaponSearchResults.innerHTML = results
      .map((entry) => {
        const safeName = escapeHtml(entry?.name || entry?.Display || `#${entry?.entry ?? ''}`);
        const itemLevel = Number(entry?.itemLevel ?? entry?.ItemLevel);
        const quality = Number(entry?.quality ?? entry?.Quality);
        const inventoryType = entry?.inventoryType ?? entry?.InventoryType;
        const summary = [
          entry?.entry != null ? `#${entry.entry}` : '',
          Number.isFinite(itemLevel) ? `ilvl ${itemLevel}` : '',
          inventoryType != null ? `inv ${inventoryType}` : '',
          Number.isFinite(quality) ? `quality ${quality}` : '',
        ]
          .filter(Boolean)
          .join(' · ');
        return `
          <article class="rounded-2xl border border-white/5 bg-gray-900/60 p-3 text-sm text-indigo-100/90 flex items-center justify-between gap-3">
            <div>
              <p class="font-semibold text-white">${safeName}</p>
              <p class="text-xs text-indigo-200/80">${summary || 'Weapon template'}</p>
            </div>
            <button type="button" class="inline-flex items-center rounded-2xl border border-indigo-400/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.25em] text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400" data-weapon-entry="${escapeHtml(entry?.entry ?? entry?.Entry ?? '')}">
              Use
            </button>
          </article>
        `;
  ...
  function populateWeaponBranchSelect() {
    if (!weaponBranchSelect) return;
    const entries = Object.values(WEAPON_BRANCH_META);
    if (entries.length) {
      weaponBranchSelect.innerHTML = entries
        .map(
          (branch) =>
            `<option value="${escapeHtml(branch.key)}">${escapeHtml(branch.label || branch.key)}</option>`
        )
        .join('');
    } else {
      weaponBranchSelect.innerHTML = `
        <option value="retail">Retail</option>
        <option value="classic">Classic</option>
      `;
    }
    weaponBranchSelect.value = weaponState.branch || DEFAULT_WEAPON_BRANCH;
  }

  function updateWeaponBranchAccess(availableRealms) {
    if (!weaponBranchSelect) return;
    const allowed = Array.isArray(availableRealms) && availableRealms.length ? availableRealms : null;
    Array.from(weaponBranchSelect.options).forEach((option) => {
      const value = normalizeWeaponBranchInput(option.value);
      const enabled = !allowed || allowed.includes(value);
      option.disabled = !enabled;
    });
    if (allowed && !allowed.includes(weaponState.branch)) {
      weaponState.branch = allowed[0];
      weaponBranchSelect.value = weaponState.branch;
      persistStoredValue(STORAGE_KEYS.weaponBranch, weaponState.branch);
      resetWeaponBlueprint(true);
    }
  }

  function renderQuestSearchResults() {
    if (!questSearchResults) return;
    const results = Array.isArray(questState.searchResults) ? questState.searchResults : [];
    if (!results.length) {
      questSearchResults.innerHTML =
        '<p class="text-sm text-indigo-200/70">No quests matched your search.</p>';
      return;
    }
    questSearchResults.innerHTML = results
      .map((entry) => {
        const safeTitle = escapeHtml(entry?.Title || entry?.title || `Quest #${entry?.ID ?? entry?.entry ?? ''}`);
        const questLevel = Number(entry?.QuestLevel ?? entry?.questlevel);
        const minLevel = Number(entry?.MinLevel ?? entry?.minlevel);
        const summary = [
          entry?.ID != null ? `#${entry.ID}` : entry?.entry != null ? `#${entry.entry}` : '',
          Number.isFinite(questLevel) ? `level ${questLevel}` : '',
          Number.isFinite(minLevel) ? `min ${minLevel}` : '',
        ]
          .filter(Boolean)
          .join(' · ');
        const entryId = entry?.ID ?? entry?.entry ?? '';
        return `
          <article class="rounded-2xl border border-white/5 bg-gray-900/60 p-3 text-sm text-indigo-100/90 flex items-center justify-between gap-3">
            <div>
              <p class="font-semibold text-white">${safeTitle}</p>
              <p class="text-xs text-indigo-200/80">${summary || 'Quest template'}</p>
            </div>
            <button type="button" class="inline-flex items-center rounded-2xl border border-indigo-400/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.25em] text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400" data-quest-entry="${escapeHtml(entryId)}">
              Use
            </button>
          </article>
        `;
  ...
  function renderQuestActorSearchResults() {
    if (!questActorSearchResults) return;
    const results = Array.isArray(questState.actor.searchResults) ? questState.actor.searchResults : [];
    if (!results.length) {
      questActorSearchResults.innerHTML =
        '<p class="text-sm text-indigo-200/70">No actor matches yet.</p>';
      return;
    }
    questActorSearchResults.innerHTML = results
      .map((entry) => {
        const safeName = escapeHtml(entry?.name || entry?.Name || `Entry #${entry?.entry ?? ''}`);
        const subname = entry?.subname || entry?.SubName;
        const summaryParts = [
          entry?.entry != null ? `#${entry.entry}` : '',
          entry?.type != null ? `type ${entry.type}` : '',
        ].filter(Boolean);
        if (subname) summaryParts.push(subname);
        const entryId = entry?.entry ?? entry?.Entry ?? '';
        return `
          <article class="rounded-2xl border border-white/5 bg-gray-900/60 p-3 text-sm text-indigo-100/90 flex items-center justify-between gap-3">
            <div>
              <p class="font-semibold text-white">${safeName}</p>
              <p class="text-xs text-indigo-200/80">${summaryParts.join(' · ')}</p>
            </div>
            <button type="button" class="inline-flex items-center rounded-2xl border border-indigo-400/60 px-3 py-1 text-xs font-semibold uppercase tracking-[0.25em] text-indigo-100 transition hover:border-indigo-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-indigo-400" data-actor-entry="${escapeHtml(entryId)}">
              Use
            </button>
          </article>
        `;
  ...
  function formatSummaryList(pairs) {
    const items = pairs
      .filter((pair) => pair && pair.label)
      .map(
        (pair) => `
          <div class="flex flex-col rounded-xl border border-white/5 bg-black/20 p-3">
            <span class="text-xs font-semibold uppercase tracking-[0.35em] text-indigo-300">${escapeHtml(pair.label)}</span>
            <span class="text-sm font-semibold text-white">${escapeHtml(pair.value ?? '—')}</span>
          </div>
        `
      )
      .join('');
    return `<div class="grid gap-3 sm:grid-cols-2">${items}</div>`;
  }

  function formatWeaponSummary(summary) {
    if (!summary) return '';
    const blocks = formatSummaryList([
      { label: 'Entry', value: summary.entry ?? summary.ID ?? '—' },
      { label: 'Name', value: summary.name ?? summary.Display ?? '—' },
      { label: 'Quality', value: summary.quality },
      { label: 'Item level', value: summary.itemLevel },
      { label: 'Inventory type', value: summary.inventoryType },
      { label: 'Display/Icon', value: summary.displayId ?? summary.iconFileDataId },
    ]);
    return `
      <div class="rounded-2xl border border-white/10 bg-black/30 p-4">
        <p class="text-sm font-semibold uppercase tracking-[0.35em] text-indigo-300 mb-2">Source weapon</p>
        ${blocks}
      </div>
    `;
  }

  function renderWeaponBlueprint() {
    if (!weaponFieldGrid) return;
    if (!weaponState.tables.length) {
      weaponFieldGrid.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to load a source weapon.</div>';
      weaponJsonEditors = new Map();
      return;
    }
    const editors = weaponState.tables
      .map((table) => {
        const editorId = `weaponJsonEditor-${table.name}`;
        const prettyJson = JSON.stringify(table.data || {}, null, 2);
        return `
          <div class="rounded-2xl border border-white/10 bg-black/30 p-4 space-y-2">
            <label class="block text-sm font-medium text-indigo-200" for="${editorId}">${escapeHtml(table.label || table.name)} payload</label>
            <textarea id="${editorId}" rows="14" class="glow-input w-full rounded-2xl p-3 text-[13px] font-mono leading-snug focus:outline-none focus:ring-2 focus:ring-indigo-400">${escapeHtml(prettyJson)}</textarea>
            <p class="text-xs text-indigo-200/70">Only differences from this table will be applied during cloning.</p>
          </div>
        `;
      })
      .join('');
    weaponFieldGrid.innerHTML = `
      ${formatWeaponSummary(weaponState.summary)}
      ${editors}
    `;
    weaponJsonEditors = new Map();
    weaponState.tables.forEach((table) => {
      const editor = document.getElementById(`weaponJsonEditor-${table.name}`);
      if (editor) {
        weaponJsonEditors.set(table.name, editor);
      }
    });
    if (weaponResetButton) {
      weaponResetButton.disabled = false;
    }
  }

  function formatQuestSummary(source) {
    if (!source) return '';
    const summary = formatSummaryList([
      { label: 'Quest ID', value: source.ID ?? source.entry ?? '—' },
      { label: 'Title', value: source.Title ?? source.title ?? '—' },
      { label: 'Quest level', value: source.QuestLevel ?? source.questlevel },
      { label: 'Minimum level', value: source.MinLevel ?? source.minlevel },
      { label: 'Suggested players', value: source.SuggestedPlayers ?? source.suggestedplayers },
      { label: 'Quest type', value: source.Type ?? source.QuestType },
      { label: 'Quest flags', value: source.QuestFlags ?? source.flags },
      { label: 'Previous quest', value: source.PrevQuestId ?? source.prevquestid },
    ]);
    return `
      <div class="rounded-2xl border border-white/10 bg-black/30 p-4">
        <p class="text-sm	font-semibold uppercase tracking-[0.35em] text-indigo-300 mb-2">Source quest</p>
        ${summary}
      </div>
    `;
  }

  function renderQuestBlueprint() {
    if (!questFieldGrid) return;
    if (!questState.source) {
      questFieldGrid.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to load a source quest.</div>';
      return;
    }
    const editorId = 'questJsonEditor';
    const prettyJson = questState.snapshot || JSON.stringify(questState.source, null, 2);
    questFieldGrid.innerHTML = `
      ${formatQuestSummary(questState.source)}
      <div class="rounded-2xl border border-white/10 bg-black/30 p-4 space-y-2">
        <label class="block text-sm font-medium text-indigo-200" for="${editorId}">Editable quest_template payload</label>
        <textarea id="${editorId}" rows="16" class="glow-input w-full rounded-2xl p-3 text-[13px] font-mono leading-snug focus:outline-none focus:ring-2 focus:ring-indigo-400">${escapeHtml(prettyJson)}</textarea>
        <p class="text-xs text-indigo-200/70">Change any property and press Preview to inspect the diff before writing to the world database.</p>
      </div>
    `;
    questJsonEditor = document.getElementById(editorId);
    if (questResetButton) {
      questResetButton.disabled = false;
    }
  }

  function formatActorSummary(source, typeLabel) {
    if (!source) return '';
    const summary = formatSummaryList([
      { label: 'Entry', value: source.entry ?? source.ID ?? '—' },
      { label: 'Name', value: source.name ?? source.Name ?? '—' },
      { label: 'Subname', value: source.subname ?? source.SubName ?? '' },
      { label: 'Type', value: source.type },
      { label: 'Faction', value: source.faction ?? source.Faction },
      { label: 'NPC Flags', value: source.npcflag ?? source.NpcFlags },
    ]);
    return `
      <div class="rounded-2xl border border-white/10 bg-black/30 p-4">
        <p class="text-sm font-semibold uppercase tracking-[0.35em] text-indigo-300 mb-2">${escapeHtml(typeLabel || 'Actor template')}</p>
        ${summary}
      </div>
    `;
  }

  function renderQuestActorTemplate() {
    if (!questActorTemplateFields) return;
    if (!questState.actor.enabled) {
      questActorTemplateFields.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Actor creation disabled. Toggle the switch above to enable it.</div>';
      return;
    }
    if (!questState.actor.source) {
      questActorTemplateFields.innerHTML =
        '<div class="rounded-2xl border border-white/5 bg-black/20 p-4 text-sm text-indigo-200/80">Use the search above to pick a creature or game object to clone.</div>';
      return;
    }
    const editorId = 'questActorJsonEditor';
    const prettyJson = questState.actor.snapshot || JSON.stringify(questState.actor.source, null, 2);
    const typeLabel = questActorTypeSelect?.value === 'gameobject' ? 'Game object template' : 'Creature template';
    questActorTemplateFields.innerHTML = `
      ${formatActorSummary(questState.actor.source, typeLabel)}
      <div class="rounded-2xl border border-white/10 bg-black/30 p-4 space-y-2">
        <label class="block text-sm font-medium text-indigo-200" for="${editorId}">Editable ${escapeHtml(typeLabel)} payload</label>
        <textarea id="${editorId}" rows="12" class="glow-input w-full rounded-2xl p-3 text-[13px] font-mono leading-snug focus:outline-none focus:ring-2 focus:ring-indigo-400">${escapeHtml(prettyJson)}</textarea>
        <p class="text-xs text-indigo-200/70">Only the changed columns will be written when cloning the quest giver.</p>
      </div>
    `;
    questActorJsonEditor = document.getElementById(editorId);
  }

  function parseJsonPayload(text, label, allowEmpty = false) {
    const raw = typeof text === 'string' ? text.trim() : '';
    if (!raw) {
      if (allowEmpty) return {};
      throw new Error(`${label || 'Payload'} cannot be empty.`);
    }
    try {
      ...
      throw new Error(`${label || 'JSON'} error: ${err?.message || err}`);
    }
  }

  function mergeOverrideObjects(base, extra) {
    const result = { ...base };
  ...
  function diffRecords(original, next) {
    const overrides = {};
    ...
    keys.forEach((key) => {
      if (!valuesEqual(original?.[key], next[key])) {
  ...
  function handleGmRealmChanged() {
    const realm = getActiveGmRealm();
    questState.realm = realm;
    if (!realm) {
      setWeaponSearchStatus('GM realm required to search weapons.');
      setQuestSearchStatus('GM realm required to search quests.');
      resetWeaponBlueprint();
      resetQuestBlueprint();
      resetQuestActor();
      return;
    }
    setWeaponSearchStatus(`Ready to search ${realmLabel(realm)} weapons.`);
    setQuestSearchStatus(`Ready to search ${realmLabel(realm)} quests.`);
  }

  async function handleWeaponSearch(event) {
    event?.preventDefault();
    if (!weaponSearchInput) return;
    const branch = weaponState.branch || DEFAULT_WEAPON_BRANCH;
    const query = weaponSearchInput.value.trim();
    if (!query) {
      setWeaponSearchStatus('Enter part of a weapon name or entry ID.');
      return;
    }
    if (!branch) {
      setWeaponSearchStatus('Select a weapon branch to search.');
      return;
    }
    setWeaponSearchStatus(`Searching ${weaponBranchLabel(branch)} weapons…`);
    weaponState.loading = true;
    weaponState.searchResults = [];
    renderWeaponSearchResults();
    try {
      ...
      const res = await fetch(`${gmWeaponSearchEndpoint}?${params.toString()}`, {
        credentials: 'same-origin',
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setWeaponSearchStatus(data?.error || 'Unable to search weapons.');
        return;
      }
      weaponState.searchResults = Array.isArray(data?.weapons) ? data.weapons : [];
      if (!weaponState.searchResults.length) {
        setWeaponSearchStatus('No weapons matched your search.');
      } else {
        setWeaponSearchStatus(`Loaded ${weaponState.searchResults.length} weapons.`);
      }
      renderWeaponSearchResults();
    } catch (err) {
      console.error('Weapon search failed', err);
      setWeaponSearchStatus('Network error while searching weapons.');
    } finally {
      weaponState.loading = false;
    }
  }

  async function loadWeaponBlueprint(entryValue) {
    const branch = weaponState.branch || DEFAULT_WEAPON_BRANCH;
    if (!branch) {
      setWeaponStatus('Select a weapon branch to load weapons.');
      return;
    }
    const numericEntry = Number(entryValue);
    if (!Number.isFinite(numericEntry)) {
      setWeaponStatus('Invalid weapon entry.');
      return;
    }
    setWeaponStatus(`Loading weapon #${numericEntry} from ${weaponBranchLabel(branch)}…`);
    try {
      const res = await fetch(
        `${gmWeaponDetailEndpoint}/${encodeURIComponent(
          numericEntry
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setWeaponStatus(data?.error || 'Unable to load weapon.');
        return;
      }
      if (!data?.tables?.length) {
        setWeaponStatus('Weapon not found.');
        return;
      }
      weaponState.branch = branch;
      weaponState.summary = data.summary || null;
      weaponState.tables = Array.isArray(data.tables) ? data.tables : [];
      weaponState.suggestedEntry = data?.suggestedEntry ?? null;
      if (weaponNewEntryInput && !weaponNewEntryInput.value && weaponState.suggestedEntry != null) {
        weaponNewEntryInput.value = weaponState.suggestedEntry;
      }
      renderWeaponBlueprint();
      setWeaponStatus(`Loaded weapon #${numericEntry} from ${weaponBranchLabel(branch)}.`);
    } catch (err) {
      console.error('Weapon blueprint load failed', err);
      setWeaponStatus('Network error while loading weapon.');
    }
  }

  function collectWeaponOverrides() {
    if (!weaponState.tables.length) {
      throw new Error('Select a source weapon first.');
    }
    const payloads = [];
    for (const table of weaponState.tables) {
      const editor = weaponJsonEditors.get(table.name);
      if (!editor) continue;
      const parsed = parseJsonPayload(
        editor.value,
        `${table.label || table.name} payload`
      );
      const overrides = diffRecords(table.data, parsed);
      if (Object.keys(overrides).length) {
        payloads.push({ name: table.name, overrides });
      }
    }
    const advanced =
      weaponAdvancedOverrides && weaponAdvancedOverrides.value.trim()
        ? parseJsonPayload(weaponAdvancedOverrides.value, 'Weapon advanced overrides', true)
        : {};
    if (advanced && typeof advanced === 'object') {
      Object.entries(advanced).forEach(([tableName, additional]) => {
        if (!additional || typeof additional !== 'object') return;
        const normalized = normalizeOverrides(additional);
        if (!Object.keys(normalized).length) return;
        const existing = payloads.find((entry) => entry.name === tableName);
        if (existing) {
          existing.overrides = mergeOverrideObjects(existing.overrides, normalized);
        } else {
          payloads.push({ name: tableName, overrides: normalized });
        }
      });
    }
    return payloads;
  ...
  function updateWeaponPreview() {
    try {
      const tables = collectWeaponOverrides();
      const sourceEntry = weaponState.summary?.entry ?? weaponState.summary?.ID;
      const newEntry = Number(weaponNewEntryInput?.value || weaponState.suggestedEntry);
      const payload = {
        branch: weaponState.branch,
        sourceEntry,
        newEntry: Number.isFinite(newEntry) ? newEntry : null,
        tables,
      };
      if (!payload.newEntry) {
        throw new Error('Enter a numeric entry ID for the cloned weapon.');
      }
      if (!payload.sourceEntry) {
        throw new Error('Load a source weapon before previewing changes.');
      }
      if (!Array.isArray(tables) || !tables.length) {
        throw new Error('No changes detected. Adjust one of the table payloads before previewing.');
      }
      if (weaponPreviewOutput) {
        weaponPreviewOutput.textContent = JSON.stringify(payload, null, 2);
      }
      setWeaponStatus(`Preview ready with ${tables.length} table(s) modified.`);
    } catch (err) {
      if (weaponPreviewOutput) {
        weaponPreviewOutput.textContent = String(err?.message || err);
      }
      setWeaponStatus(err?.message || 'Unable to build preview.');
    }
  }

  async function submitWeaponClone() {
    try {
      const branch = weaponState.branch;
      if (!branch) {
        throw new Error('Select a weapon branch first.');
      }
      const sourceEntry = weaponState.summary?.entry ?? weaponState.summary?.ID;
      if (!sourceEntry) {
        throw new Error('Load a source weapon before cloning.');
      }
      const newEntry = Number(weaponNewEntryInput?.value || weaponState.suggestedEntry);
      if (!Number.isInteger(newEntry) || newEntry <= 0) {
        throw new Error('Enter a valid numeric entry ID for the new weapon.');
      }
      const tables = collectWeaponOverrides();
      if (!tables.length) {
        throw new Error('No changes detected. Adjust the JSON payload before cloning.');
      }
      setWeaponSubmitting(true);
      setWeaponStatus(`Cloning weapon #${sourceEntry} into ${newEntry} on ${weaponBranchLabel(branch)}…`);
      const res = await fetch(gmWeaponCreateEndpoint, {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(data?.error || 'Unable to clone weapon.');
      }
      if (weaponPreviewOutput) {
        weaponPreviewOutput.textContent = JSON.stringify(data, null, 2);
      }
      setWeaponStatus(`Weapon cloned as #${data?.entry ?? newEntry}.`);
    } catch (err) {
      setWeaponStatus(err?.message || 'Weapon clone failed.');
    } finally {
      setWeaponSubmitting(false);
    }
  }

  async function handleQuestSearch(event) {
    event?.preventDefault();
    if (!questSearchInput) return;
    const query = questSearchInput.value.trim();
    if (!query) {
      setQuestSearchStatus('Enter part of a quest title or ID.');
      return;
    }
    const realm = getActiveGmRealm();
    if (!realm) {
      setQuestSearchStatus('Select a GM realm to search.');
      return;
    }
    setQuestSearchStatus(`Searching ${realmLabel(realm)} quests…`);
    questState.searchResults = [];
    renderQuestSearchResults();
    try {
      const params = new URLSearchParams({
      ...
      const res = await fetch(`${gmQuestSearchEndpoint}?${params.toString()}`, {
        credentials: 'same-origin',
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setQuestSearchStatus(data?.error || 'Unable to search quests.');
        return;
      }
      questState.realm = realm;
      questState.searchResults = Array.isArray(data?.quests) ? data.quests : [];
      if (!questState.searchResults.length) {
        setQuestSearchStatus('No quests matched your search.');
      } else {
        setQuestSearchStatus(`Loaded ${questState.searchResults.length} quests.`);
      }
      renderQuestSearchResults();
    } catch (err) {
      console.error('Quest search failed', err);
      setQuestSearchStatus('Network error while searching quests.');
    }
  }

  async function loadQuestBlueprint(entryValue) {
    const realm = getActiveGmRealm();
    if (!realm) {
      setQuestStatus('Select a GM realm to load quests.');
      return;
    }
    const numericEntry = Number(entryValue);
    if (!Number.isFinite(numericEntry)) {
      setQuestStatus('Invalid quest ID.');
      return;
    }
    setQuestStatus(`Loading quest #${numericEntry}…`);
    try {
      const res = await fetch(
        `${gmQuestDetailEndpoint}/${encodeURIComponent(numericEntry)}?realm=${encodeURIComponent(realm)}`,
        { credentials: 'same-origin' }
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setQuestStatus(data?.error || 'Unable to load quest.');
        return;
      }
      if (!data?.quest) {
        setQuestStatus('Quest not found.');
        return;
      }
      questState.realm = realm;
      questState.source = data.quest;
      questState.snapshot = JSON.stringify(data.quest, null, 2);
      questState.suggestedEntry = data?.suggestedEntry ?? null;
      if (questNewEntryInput && !questNewEntryInput.value && questState.suggestedEntry != null) {
        questNewEntryInput.value = questState.suggestedEntry;
      }
      renderQuestBlueprint();
      setQuestStatus(`Loaded quest #${numericEntry}.`);
    } catch (err) {
      console.error('Quest blueprint load failed', err);
      setQuestStatus('Network error while loading quest.');
    }
  }

  async function handleQuestActorSearch(event) {
    event?.preventDefault();
    if (!questActorSearchInput) return;
    const query = questActorSearchInput.value.trim();
    if (!query) {
      setQuestActorStatus('Enter an NPC or game object name to search.');
      return;
    }
    const realm = getActiveGmRealm();
    if (!realm) {
      setQuestActorStatus('Select a GM realm to search for actors.');
      return;
    }
    const type = questActorTypeSelect?.value === 'gameobject' ? 'gameobject' : 'creature';
    questState.actor.type = type;
    setQuestActorStatus(`Searching ${realmLabel(realm)} ${type === 'creature' ? 'creatures' : 'game objects'}…`);
    questState.actor.searchResults = [];
    renderQuestActorSearchResults();
    try {
      const params = new URLSearchParams({
      ...
      const res = await fetch(`${gmActorSearchEndpoint}?${params.toString()}`, {
        credentials: 'same-origin',
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setQuestActorStatus(data?.error || 'Unable to search actors.');
        return;
      }
      questState.actor.searchResults = Array.isArray(data?.results) ? data.results : [];
      if (!questState.actor.searchResults.length) {
        setQuestActorStatus('No actors matched your search.');
      } else {
        setQuestActorStatus(`Loaded ${questState.actor.searchResults.length} templates.`);
      }
      renderQuestActorSearchResults();
    } catch (err) {
      console.error('Actor search failed', err);
      setQuestActorStatus('Network error while searching actors.');
    }
  }

  async function loadQuestActorTemplate(entryValue) {
    const realm = getActiveGmRealm();
    if (!realm) {
      setQuestActorStatus('Select a GM realm to load actors.');
      return;
    }
    const numericEntry = Number(entryValue);
    if (!Number.isFinite(numericEntry)) {
      setQuestActorStatus('Invalid actor entry.');
      return;
    }
    const type = questActorTypeSelect?.value === 'gameobject' ? 'gameobject' : 'creature';
    questState.actor.type = type;
    setQuestActorStatus(`Loading ${type} #${numericEntry}…`);
    try {
      const res = await fetch(
        `${gmActorDetailEndpoint}/${encodeURIComponent(type)}/${encodeURIComponent(numericEntry)}?realm=${encodeURIComponent(
          realm
        )}`,
        { credentials: 'same-origin' }
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setQuestActorStatus(data?.error || 'Unable to load actor template.');
        return;
      }
      if (!data?.template) {
        setQuestActorStatus('Actor template not found.');
        return;
      }
      questState.actor.source = data.template;
      questState.actor.snapshot = JSON.stringify(data.template, null, 2);
      questState.actor.suggestedEntry = data?.suggestedEntry ?? null;
      if (questActorNewEntryInput && !questActorNewEntryInput.value && questState.actor.suggestedEntry != null) {
        questActorNewEntryInput.value = questState.actor.suggestedEntry;
      }
      if (questActorSourceInput) {
        questActorSourceInput.value = numericEntry;
      }
      renderQuestActorTemplate();
      setQuestActorStatus(`Loaded ${type === 'creature' ? 'creature' : 'game object'} #${numericEntry}.`);
    } catch (err) {
      console.error('Actor load failed', err);
      setQuestActorStatus('Network error while loading actor.');
    }
  }

  function collectQuestOverrides() {
    if (!questState.source) {
      throw new Error('Select a source quest first.');
    }
    if (!questJsonEditor) {
      throw new Error('Quest editor not initialized.');
    }
    const parsed = parseJsonPayload(questJsonEditor.value, 'Quest blueprint');
    const overrides = diffRecords(questState.source, parsed);
    const advanced =
      questAdvancedOverrides && questAdvancedOverrides.value.trim()
        ? parseJsonPayload(questAdvancedOverrides.value, 'Quest advanced overrides', true)
        : {};
    return mergeOverrideObjects(overrides, advanced);
  }

  function collectQuestActorOverrides() {
    if (!questState.actor.enabled) {
      return {};
    }
    if (!questState.actor.source || !questActorJsonEditor) {
      return {};
    }
    const parsed = parseJsonPayload(questActorJsonEditor.value, 'Actor blueprint');
    const advanced =
      questActorAdvancedOverrides && questActorAdvancedOverrides.value.trim()
        ? parseJsonPayload(questActorAdvancedOverrides.value, 'Actor advanced overrides', true)
        : {};
    return mergeOverrideObjects(diffRecords(questState.actor.source, parsed), advanced);
  }

  function buildActorSpawnPayload() {
    return {
      map: Number(questActorSpawnMap?.value ?? 0) || 0,
      zoneId: Number(questActorSpawnZone?.value ?? 0) || 0,
      areaId: Number(questActorSpawnArea?.value ?? 0) || 0,
      position_x: Number(questActorSpawnX?.value ?? 0) || 0,
      position_y: Number(questActorSpawnY?.value ?? 0) || 0,
      position_z: Number(questActorSpawnZ?.value ?? 0) || 0,
      orientation: Number(questActorSpawnO?.value ?? 0) || 0,
      phaseMask: Number(questActorSpawnPhase?.value ?? 1) || 1,
      spawnMask: Number(questActorSpawnMask?.value ?? 1) || 1,
    };
  }
  ...
  function buildQuestPayloads() {
    const realm = getActiveGmRealm();
    if (!realm) {
      throw new Error('Select a GM realm first.');
    }
    const questSourceEntry = questState.source?.ID ?? questState.source?.entry;
    if (!questSourceEntry) {
      throw new Error('Load a source quest first.');
    }
    const questOverrides = collectQuestOverrides();
    const newQuestEntry = Number(questNewEntryInput?.value || questState.suggestedEntry);
    if (!Number.isInteger(newQuestEntry) || newQuestEntry <= 0) {
      throw new Error('Enter a valid numeric quest ID for the clone.');
    }
    const payload = {
      realm,
      quest: {
        sourceEntry: questSourceEntry,
        newEntry: newQuestEntry,
        overrides: questOverrides,
      },
    };
    if (questActorToggle && !questActorToggle.checked) {
      payload.actor = { enabled: false };
      return payload;
    }
    const actorType = questActorTypeSelect?.value === 'gameobject' ? 'gameobject' : 'creature';
    const actorSourceEntry = Number(questActorSourceInput?.value);
    const actorNewEntry = Number(questActorNewEntryInput?.value || questState.actor.suggestedEntry);
    if (!Number.isInteger(actorSourceEntry) || actorSourceEntry <= 0) {
      throw new Error('Select a source actor entry to clone.');
    }
    if (!Number.isInteger(actorNewEntry) || actorNewEntry <= 0) {
      throw new Error('Enter a valid numeric entry for the cloned actor.');
    }
    payload.actor = {
      enabled: true,
      type: actorType,
      sourceEntry: actorSourceEntry,
      newEntry: actorNewEntry,
      overrides: collectQuestActorOverrides(),
      spawn: buildActorSpawnPayload(),
      addStarter: questActorStarterToggle?.checked !== false,
      addEnder: questActorEnderToggle?.checked !== false,
    };
    return payload;
  }
  ...
  function updateQuestPreview() {
    try {
      const payload = buildQuestPayloads();
      if (questPreviewOutput) {
        questPreviewOutput.textContent = JSON.stringify(payload, null, 2);
      }
      setQuestStatus(
        `Preview ready with ${Object.keys(payload.quest?.overrides || {}).length} quest change(s).`
      );
    } catch (err) {
      if (questPreviewOutput) {
        questPreviewOutput.textContent = String(err?.message || err);
      }
      setQuestStatus(err?.message || 'Unable to build preview.');
    }
  }

  async function submitQuestCreation() {
    try {
      const payload = buildQuestPayloads();
      if (!Object.keys(payload.quest?.overrides || {}).length) {
        throw new Error('No quest fields were changed. Update the JSON payload before cloning.');
      }
      if (payload.actor?.enabled && !Object.keys(payload.actor.overrides || {}).length) {
        setQuestActorStatus('No actor changes detected. The template will still be cloned with defaults.');
      }
      setQuestSubmitting(true);
      setQuestStatus('Cloning quest and actor…');
      const res = await fetch(gmQuestCreateEndpoint, {
        method: 'POST',
        credentials: 'same-origin',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      ...
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(data?.error || 'Unable to clone quest.');
      }
      if (questPreviewOutput) {
        questPreviewOutput.textContent = JSON.stringify(data, null, 2);
      }
      setQuestStatus(`Quest cloned as #${data?.quest?.entry ?? payload.quest.newEntry}.`);
      if (payload.actor?.enabled) {
        setQuestActorStatus(
          `Actor cloned as #${data?.actor?.entry ?? payload.actor.newEntry} at GUID ${data?.actor?.guid ?? 'new'}.`
        );
      }
    } catch (err) {
      setQuestStatus(err?.message || 'Quest clone failed.');
    } finally {
  ...
  function setActiveGmTool(toolId) {
    const normalized = toolId === 'weapon' ? 'weapon' : 'server';
    activeGmTool = normalized;
    gmToolPanels.forEach((panel) => {
      const panelId = panel.getAttribute('data-gmtool-panel') || '';
      const matches = panelId === normalized;
      panel.classList.toggle('hidden', !matches);
    });
    gmToolTabButtons.forEach((button) => {
      const target = button.getAttribute('data-gmtool-tab');
      const matches = target === normalized;
      button.setAttribute('aria-selected', matches ? 'true' : 'false');
      button.classList.toggle('text-indigo-100', matches);
      button.classList.toggle('text-indigo-100/70', !matches);
      button.classList.toggle('bg-gradient-to-r', matches);
  ...
  gmToolTabButtons.forEach((button) => {
    button.addEventListener('click', () => {
      const target = button.getAttribute('data-gmtool-tab');
      if (!target) return;
      setActiveGmTool(target);
    });
  });

  setActiveTab(activeTabId);
  setActiveGmTool(activeGmTool);
  handleGmRealmChanged();
  ...
  weaponBranchSelect?.addEventListener('change', (event) => {
    const nextBranch = normalizeWeaponBranchInput(event.target?.value);
    if (!nextBranch || nextBranch === weaponState.branch) {
      weaponBranchSelect.value = weaponState.branch || DEFAULT_WEAPON_BRANCH;
      return;
    }
    weaponState.branch = nextBranch;
    persistStoredValue(STORAGE_KEYS.weaponBranch, weaponState.branch);
    resetWeaponBlueprint(true);
    setWeaponSearchStatus(`Switched to ${weaponBranchLabel(nextBranch)} weapons.`);
  });
  weaponSearchForm?.addEventListener('submit', handleWeaponSearch);
  weaponSearchReset?.addEventListener('click', (event) => {
    event.preventDefault();
    ...
    weaponState.searchResults = [];
    renderWeaponSearchResults();
    setWeaponSearchStatus('Search cleared.');
  });
  weaponSearchResults?.addEventListener('click', (event) => {
    const button = event.target?.closest('button[data-weapon-entry]');
    if (!button) return;
    const entry = button.getAttribute('data-weapon-entry');
    if (entry) {
      loadWeaponBlueprint(entry);
    }
  });
  weaponPreviewButton?.addEventListener('click', (event) => {
    event.preventDefault();
    updateWeaponPreview();
  });
  weaponCreateButton?.addEventListener('click', (event) => {
    event.preventDefault();
    submitWeaponClone();
  });
  weaponResetButton?.addEventListener('click', (event) => {
    event.preventDefault();
    if (!weaponState.tables.length) {
      resetWeaponBlueprint();
      return;
    ...
    weaponState.tables.forEach((table) => {
      const editor = weaponJsonEditors.get(table.name);
      if (editor) {
        editor.value = JSON.stringify(table.data || {}, null, 2);
      }
    });
    setWeaponStatus('Blueprint reset to the source state.');
  });

  questSearchForm?.addEventListener('submit', handleQuestSearch);
  questSearchReset?.addEventListener('click', (event) => {
    event.preventDefault();
    if (questSearchInput) {
    ...
    questState.searchResults = [];
    renderQuestSearchResults();
    setQuestSearchStatus('Search cleared.');
  });
  questSearchResults?.addEventListener('click', (event) => {
    const button = event.target?.closest('button[data-quest-entry]');
    if (!button) return;
    const entry = button.getAttribute('data-quest-entry');
    if (entry) {
      loadQuestBlueprint(entry);
    }
  });
  questPreviewButton?.addEventListener('click', (event) => {
    event.preventDefault();
    updateQuestPreview();
  });
  questCreateButton?.addEventListener('click', (event) => {
    event.preventDefault();
    submitQuestCreation();
  });
  questResetButton?.addEventListener('click', (event) => {
    event.preventDefault();
    if (questJsonEditor) {
      questJsonEditor.value = questState.snapshot || questJsonEditor.value;
      setQuestStatus('Blueprint reset to the source state.');
    } else {
      resetQuestBlueprint();
    }
  });

  questActorSearchForm?.addEventListener('submit', handleQuestActorSearch);
  questActorSearchResults?.addEventListener('click', (event) => {
    const button = event.target?.closest('button[data-actor-entry]');
    if (!button) return;
    const entry = button.getAttribute('data-actor-entry');
    if (entry) {
      loadQuestActorTemplate(entry);
    }
  });
  questActorToggle?.addEventListener('change', () => {
    const enabled = questActorToggle.checked;
    questState.actor.enabled = enabled;
    if (questActorFieldsWrapper) {
      questActorFieldsWrapper.classList.toggle('opacity-60', !enabled);
    }
    setQuestActorStatus(
      enabled ? 'Auto-creation enabled.' : 'Actor creation disabled. Quest will not spawn a new giver.'
    );
    if (!enabled) {
      resetQuestActor();
    } else {
  ...
  questActorTypeSelect?.addEventListener('change', () => {
    questState.actor.type = questActorTypeSelect.value === 'gameobject' ? 'gameobject' : 'creature';
    if (questState.actor.source) {
      // reset blueprint if type changed
      resetQuestActor();
    }
  });
...
const WORLD_TABLE_ALLOWLIST = new Set([
  'item_template',
  'item_template_addon',
...
const WORLD_TABLE_SCHEMA_CACHE = new Map();

async function describeWorldTable(table) {
  const normalized = String(table || '').toLowerCase();
  if (!WORLD_TABLE_ALLOWLIST.has(normalized)) {
    throw new Error(`Table ${table} is not available for automation`);
  }
  if (WORLD_TABLE_SCHEMA_CACHE.has(normalized)) {
    return WORLD_TABLE_SCHEMA_CACHE.get(normalized);
  }
  const pools = [worldPool, classicWorldPool, hotfixPool].filter(Boolean);
  let schema = null;
  for (const poolRef of pools) {
    try {
      const [rows] = await poolRef.query(`SHOW COLUMNS FROM \`${table}\``);
      schema = rows.map((row) => ({
        name: row.Field,
      }));
      break;
  ...
  if (!schema) {
    throw new Error(`Unable to describe table ${table}`);
  }
  WORLD_TABLE_SCHEMA_CACHE.set(normalized, schema);
  return schema;
}

async function fetchSingleRow(client, table, keyColumn, keyValue) {
...
async function ensureEntryAvailable(client, table, keyColumn, keyValue) {
  const existing = await fetchSingleRow(client, table, keyColumn, keyValue);
  if (existing) {
    throw new PortalHttpError(`Entry ${keyValue} already exists in ${table}`, 409);
  }
}
...
function normalizeOverrides(overrides) {
  ...
  for (const [key, rawValue] of Object.entries(overrides)) {
    ...
    payload[key] = sanitizeDbValue(rawValue);
  }
  return payload;
}

function buildLikePattern(text) {
  const safe = String(text ?? '').replace(/[%_\\]/g, (c) => `\\${c}`);
  return `%${safe}%`;
}

function buildClonedRow(schema, sourceRow, overrides, keyColumn, keyValue) {
  const row = {};
  for (const column of schema) {
    const columnName = column.name;
    if (columnName === keyColumn) {
      row[columnName] = keyValue;
      continue;
    }
...
async function cloneTableRecords(client, table, keyColumn, sourceValue, targetValue) {
  let schema;
  try {
    schema = await describeWorldTable(table);
  } catch (err) {
    return 0;
  }
  const [rows] = await client.execute(
  ...
  for (const row of rows) {
    const cloned = buildClonedRow(schema, row, {}, keyColumn, targetValue);
    await insertRow(client, table, cloned);
  }
  return rows.length;
}

async function runInTransaction(poolRef, handler) {
...
async function allocateNextGuid(client, table) {
  const [rows] = await client.execute(`SELECT MAX(guid) AS maxGuid FROM \`${table}\``);
  const next = (toSafeNumber(rows?.[0]?.maxGuid) || 0) + 1;
  return next;
}

async function reloadTablesForRealm(realm, tables = []) {
  if (!tables || !tables.length) {
    return [];
  }
  const exec = realm === 'classic' ? executeClassicCommand : executeRetailCommand;
  const soap = realm === 'classic' ? CLASSIC_SOAP : SOAP;
  const results = [];
  for (const table of tables) {
    const command = `reload ${table}`;
    try {
      const resp = await exec({ soap, command });
      results.push({
        table,
        ok: true,
        response: resp?.ret ?? resp?.raw ?? null,
      });
    } catch (err) {
      results.push({
        table,
        ok: false,
        error: err?.message || 'SOAP reload failed',
      });
    }
  }
  return results;
...
const CLASSIC_EDITABLE_TABLES = [
  { name: 'item_template', keyColumn: 'entry', label: 'item_template', required: true },
];

const CLASSIC_AUTO_TABLES = [
  { name: 'item_template_addon', keyColumn: 'Id', label: 'item_template_addon' },
  { name: 'item_template_locale', keyColumn: 'ID', label: 'item_template_locale' },
];

const RETAIL_EDITABLE_TABLES = [
  { name: 'item_sparse', keyColumn: 'ID', label: 'item_sparse', required: true },
  { name: 'item', keyColumn: 'ID', label: 'item' },
];

const RETAIL_AUTO_TABLES = [
  { name: 'item_sparse_locale', keyColumn: 'ID', label: 'item_sparse_locale' },
  { name: 'item_search_name', keyColumn: 'ID', label: 'item_search_name' },
  { name: 'item_search_name_locale', keyColumn: 'ID', label: 'item_search_name_locale' },
  { name: 'item_name_description', keyColumn: 'ID', label: 'item_name_description' },
  { name: 'item_name_description_locale', keyColumn: 'ID', label: 'item_name_description_locale' },
];

const RETAIL_CHILD_TABLES = [
  {
    name: 'item_modified_appearance',
    keyColumn: 'ItemID',
    idColumn: 'ID',
    label: 'item_modified_appearance',
    propagate: [{ table: 'item_modified_appearance_extra', keyColumn: 'ID', label: 'item_modified_appearance_extra' }],
  },
  {
    name: 'item_x_item_effect',
    keyColumn: 'ItemID',
    idColumn: 'ID',
    label: 'item_x_item_effect',
  },
];

const WEAPON_BRANCHES = {
  retail: {
    key: 'retail',
    label: WEAPON_BRANCH_UI.retail?.label || 'Retail weapons',
    gmRealm: 'retail',
    getPool: () => hotfixPool || worldPool,
    allocate: { table: 'item_sparse', column: 'ID' },
    editableTables: RETAIL_EDITABLE_TABLES,
    autoTables: RETAIL_AUTO_TABLES,
    childTables: RETAIL_CHILD_TABLES,
    reloadTables: ['item_sparse', 'item'],
  },
  classic: {
    key: 'classic',
    label: WEAPON_BRANCH_UI.classic?.label || 'Classic weapons',
    gmRealm: 'classic',
    getPool: () => classicWorldPool || worldPool,
    allocate: { table: 'item_template', column: 'entry' },
    editableTables: CLASSIC_EDITABLE_TABLES,
    autoTables: CLASSIC_AUTO_TABLES,
    childTables: [],
    reloadTables: ['item_template'],
  },
};

function normalizeWeaponBranch(value, fallback = null) {
  const text = typeof value === 'string' ? value.trim().toLowerCase() : '';
  if (text === 'classic') return 'classic';
  if (text === 'retail') return 'retail';
  if (fallback == null) return null;
  return normalizeWeaponBranch(fallback, null);
}

function getWeaponBranchConfig(branch) {
  const normalized = normalizeWeaponBranch(branch);
  if (!normalized) {
    return null;
  }
  return WEAPON_BRANCHES[normalized] || null;
}

function requireWeaponBranchConfig(branch) {
  const cfg = getWeaponBranchConfig(branch);
  if (!cfg) {
    throw new PortalHttpError('Invalid weapon branch selection', 400);
  }
  return cfg;
}

function getWeaponBranchPool(branchConfig) {
  if (!branchConfig || typeof branchConfig.getPool !== 'function') {
    throw new Error('Weapon branch misconfigured');
  }
  const poolRef = branchConfig.getPool();
  if (!poolRef) {
    throw new Error('Weapon branch database unavailable');
  }
  return poolRef;
}

async function allocateNextWeaponEntry(branchConfig) {
  const pool = getWeaponBranchPool(branchConfig);
  const table = branchConfig.allocate?.table;
  const column = branchConfig.allocate?.column;
  if (!table || !column) {
    throw new Error('Weapon branch is missing allocation metadata');
  }
  const [rows] = await pool.execute(`SELECT MAX(\`${column}\`) AS maxValue FROM \`${table}\``);
  const nextValue = (toSafeNumber(rows?.[0]?.maxValue) || 0) + 1;
  return nextValue;
}

function buildClassicWeaponSummary(row) {
  if (!row || typeof row !== 'object') {
    return null;
  }
  return {
    entry: toSafeNumber(row.entry ?? row.Entry),
    name: row.name ?? row.Name ?? '',
    quality: toSafeNumber(row.Quality),
    itemLevel: toSafeNumber(row.ItemLevel ?? row.itemlevel),
    inventoryType: toSafeNumber(row.InventoryType),
    displayId: toSafeNumber(row.displayid ?? row.DisplayID),
  };
}

function buildRetailWeaponSummary(sparseRow, itemRow) {
  if (!sparseRow || typeof sparseRow !== 'object') {
    return null;
  }
  return {
    entry: toSafeNumber(sparseRow.ID),
    name: sparseRow.Display ?? '',
    quality: toSafeNumber(sparseRow.OverallQualityID),
    itemLevel: toSafeNumber(sparseRow.ItemLevel),
    inventoryType: toSafeNumber(sparseRow.InventoryType),
    iconFileDataId: toSafeNumber(itemRow?.IconFileDataID),
  };
}

function buildWeaponSummary(branchConfig, editableMap) {
  if (!branchConfig) return null;
  if (branchConfig.key === 'classic') {
    return buildClassicWeaponSummary(editableMap.get('item_template'));
  }
  return buildRetailWeaponSummary(
    editableMap.get('item_sparse'),
    editableMap.get('item')
  );
}

async function fetchWeaponTableRows(client, tableName, keyColumn, keyValue) {
  const [rows] = await client.execute(
...
async function searchClassicWeapons(branchConfig, query, limit = 25) {
  const pool = getWeaponBranchPool(branchConfig);
  ...
  params.push(buildLikePattern(trimmed));
  const sql = `
    SELECT entry, name, Quality, ItemLevel, InventoryType, displayid
    FROM item_template
  ...
  return rows.map((row) => ({
    entry: toSafeNumber(row.entry),
    name: row.name || '',
    quality: toSafeNumber(row.Quality),
    itemLevel: toSafeNumber(row.ItemLevel),
    inventoryType: toSafeNumber(row.InventoryType),
    displayId: toSafeNumber(row.displayid),
    branch: branchConfig.key,
  }));
}

async function searchRetailWeapons(branchConfig, query, limit = 25) {
  const pool = getWeaponBranchPool(branchConfig);
  const trimmed = String(query || '').trim();
  if (!trimmed) {
    return [];
  ...
  clauses.push('s.Display LIKE ?');
  params.push(buildLikePattern(trimmed));
  const sql = `
    SELECT s.ID, s.Display, s.OverallQualityID, s.ItemLevel, s.InventoryType, i.IconFileDataID, s.VerifiedBuild
    FROM item_sparse s
    LEFT JOIN item i ON i.ID = s.ID
    WHERE ${clauses.join(' OR ')}
    ORDER BY s.VerifiedBuild DESC, s.ID DESC
    LIMIT ?
  `;
  params.push(limit * 2);
  const [rows] = await pool.execute(sql, params);
  ...
  const results = [];
  for (const row of rows) {
    const entry = toSafeNumber(row.ID);
    if (entry == null || seen.has(entry)) continue;
    seen.add(entry);
    results.push({
      entry,
      name: row.Display || '',
      quality: toSafeNumber(row.OverallQualityID),
      itemLevel: toSafeNumber(row.ItemLevel),
      inventoryType: toSafeNumber(row.InventoryType),
      iconFileDataId: toSafeNumber(row.IconFileDataID),
      branch: branchConfig.key,
    });
...
async function searchWeaponsForBranch(branchConfig, query, limit = 25) {
  if (!branchConfig) {
    return [];
  }
  if (branchConfig.key === 'classic') {
    return searchClassicWeapons(branchConfig, query, limit);
  }
  return searchRetailWeapons(branchConfig, query, limit);
}

async function loadWeaponBlueprint(branch, entry) {
  const branchConfig = requireWeaponBranchConfig(branch);
  const pool = getWeaponBranchPool(branchConfig);
  const numericEntry = toSafeNumber(entry);
  if (!Number.isInteger(numericEntry) || numericEntry <= 0) {
    throw new PortalHttpError('Invalid weapon entry.', 400);
  }
  const editableMap = new Map();
  for (const tableCfg of branchConfig.editableTables) {
    const rows = await fetchWeaponTableRows(pool, tableCfg.name, tableCfg.keyColumn, numericEntry);
    const primaryRow = rows[0] || null;
    if (!primaryRow && tableCfg.required !== false) {
      throw new PortalHttpError('Weapon not found in the selected branch.', 404);
    }
    editableMap.set(tableCfg.name, primaryRow);
  }
  if (!editableMap.size) {
    throw new PortalHttpError('Weapon not found.', 404);
  }
  const summary = buildWeaponSummary(branchConfig, editableMap);
  const tables = branchConfig.editableTables
    .map((cfg) => {
      const data = editableMap.get(cfg.name);
      if (!data) return null;
      return {
        name: cfg.name,
    ...
    .filter(Boolean);
  const suggestedEntry = await allocateNextWeaponEntry(branchConfig);
  return {
    ok: true,
    branch: branchConfig.key,
    entry: numericEntry,
...
function buildWeaponOverrideMap(tablesPayload) {
  const map = new Map();
  if (!Array.isArray(tablesPayload)) {
    return map;
  }
  for (const entry of tablesPayload) {
    if (!entry || typeof entry !== 'object') continue;
    const tableName = typeof entry.name === 'string' ? entry.name.trim().toLowerCase() : '';
    if (!tableName) continue;
    const overrides = normalizeOverrides(entry.overrides);
    if (!Object.keys(overrides).length) continue;
    map.set(tableName, overrides);
  }
  return map;
}

async function allocateSequentialIds(client, table, column, count) {
  if (!count) return [];
  const [rows] = await client.execute(`SELECT MAX(\`${column}\`) AS maxValue FROM \`${table}\``);
  let current = toSafeNumber(rows?.[0]?.maxValue) || 0;
  const values = [];
...
async function fetchRowsByColumnValues(client, table, column, values) {
  const sanitized = (values || []).filter((value) => value != null);
  if (!sanitized.length) {
    return [];
  }
...
async function cloneWeaponDefinition({ branch, sourceEntry, newEntry, tables }) {
  const branchConfig = requireWeaponBranchConfig(branch);
  const pool = getWeaponBranchPool(branchConfig);
  const sourceId = toSafeNumber(sourceEntry);
  if (!Number.isInteger(sourceId) || sourceId <= 0) {
    throw new PortalHttpError('Invalid source weapon entry.', 400);
  }
  const targetEntry = toSafeNumber(newEntry) || (await allocateNextWeaponEntry(branchConfig));
  if (!Number.isInteger(targetEntry) || targetEntry <= 0) {
    throw new PortalHttpError('Invalid new weapon entry.', 400);
  }
  const overrideMap = buildWeaponOverrideMap(tables);
  const editableSource = new Map();
  for (const tableCfg of branchConfig.editableTables) {
    const rows = await fetchWeaponTableRows(pool, tableCfg.name, tableCfg.keyColumn, sourceId);
    const row = rows[0] || null;
    if (!row && tableCfg.required !== false) {
      throw new PortalHttpError(`Source weapon missing ${tableCfg.name}`, 404);
    }
    editableSource.set(tableCfg.name, row);
  }
  if (!editableSource.get(branchConfig.editableTables[0].name)) {
    throw new PortalHttpError('Source weapon not found.', 404);
  }
  const autoSource = new Map();
  for (const tableCfg of branchConfig.autoTables) {
    const rows = await fetchWeaponTableRows(pool, tableCfg.name, tableCfg.keyColumn, sourceId);
    autoSource.set(tableCfg.name, rows);
  }
  const childSource = new Map();
  for (const tableCfg of branchConfig.childTables) {
    const rows = await fetchWeaponTableRows(pool, tableCfg.name, tableCfg.keyColumn, sourceId);
    childSource.set(tableCfg.name, rows);
  }
  await ensureEntryAvailable(pool, branchConfig.editableTables[0].name, branchConfig.editableTables[0].keyColumn, targetEntry);
  const insertedTables = [];
  await runInTransaction(pool, async (client) => {
    for (const tableCfg of branchConfig.editableTables) {
      const sourceRow = editableSource.get(tableCfg.name);
      if (!sourceRow) continue;
      const schema = await describeWorldTable(tableCfg.name);
      const overrides = overrideMap.get(tableCfg.name.toLowerCase()) || {};
      const cloned = buildClonedRow(schema, sourceRow, overrides, tableCfg.keyColumn, targetEntry);
      await insertRow(client, tableCfg.name, cloned);
      insertedTables.push({ table: tableCfg.name, inserted: 1 });
    }
    for (const tableCfg of branchConfig.autoTables) {
      const rows = autoSource.get(tableCfg.name) || [];
      if (!rows.length) continue;
      const schema = await describeWorldTable(tableCfg.name);
      let inserted = 0;
      for (const row of rows) {
        const cloned = buildClonedRow(schema, row, {}, tableCfg.keyColumn, targetEntry);
        await insertRow(client, tableCfg.name, cloned);
        inserted += 1;
      }
      insertedTables.push({ table: tableCfg.name, inserted });
    }
    for (const tableCfg of branchConfig.childTables) {
      const rows = childSource.get(tableCfg.name) || [];
      if (!rows.length) continue;
      const schema = await describeWorldTable(tableCfg.name);
      const idValues = rows
        .map((row) => toSafeNumber(row?.[tableCfg.idColumn]))
        .filter((value) => Number.isInteger(value));
      const newIds = await allocateSequentialIds(client, tableCfg.name, tableCfg.idColumn, idValues.length);
      const idMap = new Map();
      let idx = 0;
      for (const row of rows) {
        const newId = newIds[idx++] ?? null;
        if (newId == null) continue;
        idMap.set(row[tableCfg.idColumn], newId);
        const overrides = { ...row, [tableCfg.idColumn]: newId };
        const cloned = buildClonedRow(schema, row, overrides, tableCfg.keyColumn, targetEntry);
        await insertRow(client, tableCfg.name, cloned);
      }
      insertedTables.push({ table: tableCfg.name, inserted: rows.length });
      if (Array.isArray(tableCfg.propagate) && idMap.size) {
        for (const propagate of tableCfg.propagate) {
          const propRows = await fetchRowsByColumnValues(
            client,
            propagate.table,
            propagate.keyColumn,
          ...
          const propSchema = await describeWorldTable(propagate.table);
          let inserted = 0;
          for (const row of propRows) {
            const sourceId = row[propagate.keyColumn];
            const mapped = idMap.get(sourceId);
            if (mapped == null) continue;
            const cloned = buildClonedRow(propSchema, row, {}, propagate.keyColumn, mapped);
            await insertRow(client, propagate.table, cloned);
            inserted += 1;
          }
          if (inserted) {
            insertedTables.push({ table: propagate.table, inserted });
          }
        }
      }
    }
  });
  const reloads = await reloadTablesForRealm(branchConfig.gmRealm, branchConfig.reloadTables || []);
  return {
    ok: true,
    branch: branchConfig.key,
    entry: targetEntry,
    sourceEntry: sourceId,
    tablesInserted: insertedTables,
    reloads,
  };
}
...
app.get('/api/gm/weapons/search', requireSession, requireWeaponBranchQuery, async (req, res) => {
  try {
    const branch = req.gmRealm || DEFAULT_WEAPON_BRANCH;
    const branchConfig = requireWeaponBranchConfig(branch);
    ...
    const weapons = await searchWeaponsForBranch(branchConfig, query, limit);
    return res.json({
      ok: true,
      branch: branchConfig.key,
    ...
    console.error('Weapon search failed', err);
    return res.status(500).json({ error: 'Unable to search weapons' });
  }
});

app.get('/api/gm/weapons/:entry', requireSession, requireWeaponBranchQuery, async (req, res) => {
  try {
    const branch = req.gmRealm || DEFAULT_WEAPON_BRANCH;
    const detail = await loadWeaponBlueprint(branch, req.params?.entry);
    ...
    console.error('Weapon blueprint load failed', err);
    return res.status(500).json({ error: 'Unable to load weapon details' });
  }
});

app.post('/api/gm/weapons', requireSession, requireWeaponBranchBody, async (req, res) => {
  try {
    const branch = req.gmRealm || DEFAULT_WEAPON_BRANCH;
    const result = await cloneWeaponDefinition({
      branch,
      sourceEntry: req.body?.sourceEntry,
      newEntry: req.body?.newEntry,
      tables: Array.isArray(req.body?.tables) ? req.body.tables : [],
    });
    await recordPortalAuditEvent({
      portalUserId: req.session?.portal_user_id,
      action: 'gm:weapon-clone',
      details: {
    ...
    return res.status(500).json({ error: 'Unable to clone weapon' });
  }
});

  delete overrides.entry;
  try {
    await runInTransaction(poolRef, async (conn) => {
      const source = await fetchSingleRow(conn, 'item_template', 'entry', sourceEntry);
      if (!source) {
        throw new PortalHttpError('Source weapon not found', 404);
      }
      await ensureEntryAvailable(conn, 'item_template', 'entry', newEntry);
      const schema = await describeWorldTable('item_template');
      const cloned = buildClonedRow(schema, source, overrides, 'entry', newEntry);
      cloned.class = 2;
      await insertRow(conn, 'item_template', cloned);
      await cloneTableRecords(conn, 'item_template_locale', 'entry', sourceEntry, newEntry);
      await cloneTableRecords(conn, 'item_template_addon', 'entry', sourceEntry, newEntry);
    });
app.get('/api/gm/quests/search', requireSession, requireGmRealmQuery, async (req, res) => {
  const realm = req.gmRealm || normalizeRealmInput(req.query?.realm, 'retail') || 'retail';
  const poolRef = getWorldPoolForRealm(realm);
  if (!poolRef) {
    return res.status(503).json({ error: 'World database unavailable' });
  }
  ...
  const limit = Math.min(Math.max(Number(req.query?.limit) || 25, 1), 50);
  const numericEntry = toSafeNumber(rawQuery);
  const whereParts = [];
  const params = [];
  if (numericEntry != null) {
    whereParts.push('(ID = ? OR Title LIKE ? ESCAPE \'\\\\\')');
    params.push(numericEntry, buildLikePattern(rawQuery));
  } else {
    whereParts.push('Title LIKE ? ESCAPE \'\\\\\'');
    params.push(buildLikePattern(rawQuery));
  }
  params.push(limit);
  const sql = `
    SELECT ID, Title, QuestLevel, MinLevel
    FROM quest_template
  ...
  try {
    ...
    console.error('Quest search failed', err);
    return res.status(500).json({ error: 'Unable to search quests' });
  }
});

app.get('/api/gm/quests/:entry', requireSession, requireGmRealmQuery, async (req, res) => {
  const realm = req.gmRealm || normalizeRealmInput(req.query?.realm, 'retail') || 'retail';
  const poolRef = getWorldPoolForRealm(realm);
  if (!poolRef) {
    return res.status(503).json({ error: 'World database unavailable' });
  ...
  try {
    const quest = await fetchSingleRow(poolRef, 'quest_template', 'ID', entry);
    if (!quest) {
      return res.status(404).json({ error: 'Quest not found' });
    }
    const [rows] = await poolRef.execute('SELECT MAX(ID) AS maxEntry FROM quest_template');
    const suggestedEntry = Math.max((toSafeNumber(rows?.[0]?.maxEntry) || 900000) + 1, 900000);
    return res.json({
      ok: true,
      realm,
      quest,
    ...
    console.error('Quest lookup failed', err);
    return res.status(500).json({ error: 'Unable to load quest' });
  }
});

app.post('/api/gm/quests', requireSession, requireGmRealmBody, async (req, res) => {
  const realm = req.gmRealm || normalizeRealmInput(req.body?.realm, 'retail') || 'retail';
  const poolRef = getWorldPoolForRealm(realm);
  if (!poolRef) {
    return res.status(503).json({ error: 'World database unavailable' });
  ...
  const portalUserId = req.session?.portal_user_id;
  const questPayload = req.body?.quest || {};
  const questSourceEntry = toSafeNumber(questPayload?.sourceEntry);
  const questNewEntry = toSafeNumber(questPayload?.newEntry);
  if (!questSourceEntry || !questNewEntry) {
    return res.status(400).json({ error: 'Quest source and new entry IDs are required' });
  }
  const questOverrides = normalizeOverrides(questPayload?.overrides);
  delete questOverrides.ID;
  const actorPayload = req.body?.actor || {};
  const actorEnabled = actorPayload?.enabled !== false && actorPayload != null;
  try {
    const actorResult = await runInTransaction(poolRef, async (conn) => {
      const questSource = await fetchSingleRow(conn, 'quest_template', 'ID', questSourceEntry);
      if (!questSource) {
        throw new PortalHttpError('Source quest not found', 404);
      }
      await ensureEntryAvailable(conn, 'quest_template', 'ID', questNewEntry);
      const questSchema = await describeWorldTable('quest_template');
      const clonedQuest = buildClonedRow(questSchema, questSource, questOverrides, 'ID', questNewEntry);
      await insertRow(conn, 'quest_template', clonedQuest);
      await cloneTableRecords(conn, 'quest_template_locale', 'ID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_template_addon', 'ID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_offer_reward', 'ID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_request_items', 'ID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_objectives', 'QuestID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_objectives_locale', 'QuestID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_poi', 'QuestID', questSourceEntry, questNewEntry);
      await cloneTableRecords(conn, 'quest_poi_points', 'QuestID', questSourceEntry, questNewEntry);

      if (!actorEnabled) {
        return null;
      }

      const actorType = actorPayload?.type === 'gameobject' ? 'gameobject' : 'creature';
      const actorSourceEntry = toSafeNumber(actorPayload?.sourceEntry);
      const actorNewEntry = toSafeNumber(actorPayload?.newEntry);
      if (!actorSourceEntry || !actorNewEntry) {
        throw new PortalHttpError('Actor source and new entry IDs are required', 400);
      }
      const templateTable =
        actorType === 'gameobject' ? 'gameobject_template' : 'creature_template';
      const spawnTable = actorType === 'gameobject' ? 'gameobject' : 'creature';
      const starterTable =
        actorType === 'gameobject' ? 'gameobject_queststarter' : 'creature_queststarter';
      const enderTable =
        actorType === 'gameobject' ? 'gameobject_questender' : 'creature_questender';
      const actorOverrides = normalizeOverrides(actorPayload?.overrides);
      delete actorOverrides.entry;

      const actorSource = await fetchSingleRow(conn, templateTable, 'entry', actorSourceEntry);
      if (!actorSource) {
        throw new PortalHttpError('Actor template not found', 404);
      }
      await ensureEntryAvailable(conn, templateTable, 'entry', actorNewEntry);
      const actorSchema = await describeWorldTable(templateTable);
      const clonedActor = buildClonedRow(actorSchema, actorSource, actorOverrides, 'entry', actorNewEntry);
      await insertRow(conn, templateTable, clonedActor);

      const spawnGuid = await allocateNextGuid(conn, spawnTable);
      const spawn = actorPayload?.spawn || {};
      const actorSpawnRow =
        actorType === 'gameobject'
          ? {
              guid: spawnGuid,
              id: actorNewEntry,
              map: Number(spawn?.map) || 0,
              zoneId: Number(spawn?.zoneId) || 0,
              areaId: Number(spawn?.areaId) || 0,
              spawnMask: Number(spawn?.spawnMask) || 1,
              phaseMask: Number(spawn?.phaseMask) || 1,
              position_x: Number(spawn?.position_x) || 0,
              position_y: Number(spawn?.position_y) || 0,
              position_z: Number(spawn?.position_z) || 0,
              orientation: Number(spawn?.orientation) || 0,
              rotation0: 0,
              rotation1: 0,
              rotation2: Math.sin((Number(spawn?.orientation) || 0) / 2),
              rotation3: Math.cos((Number(spawn?.orientation) || 0) / 2),
              spawntimesecs: 300,
              animprogress: 0,
              state: 1,
            }
      ...
      await insertRow(conn, spawnTable, actorSpawnRow);
      if (actorPayload?.addStarter !== false) {
        await conn.execute(
          `INSERT INTO \`${starterTable}\` (id, quest) VALUES (?, ?)`,
          [actorNewEntry, questNewEntry]
        );
      }
      if (actorPayload?.addEnder !== false) {
        await conn.execute(
          `INSERT INTO \`${enderTable}\` (id, quest) VALUES (?, ?)`,
          [actorNewEntry, questNewEntry]
        );
      }
      return {
    ...
    await recordPortalAuditEvent({
      portalUserId,
      action: 'gm:quest-clone',
      details: {
        realm,
        quest: {
          sourceEntry: questSourceEntry,
          newEntry: questNewEntry,
        },
        actor: actorEnabled
          ? {
              type: actorPayload?.type || 'creature',
              sourceEntry: actorPayload?.sourceEntry,
              newEntry: actorPayload?.newEntry,
            }
          : null,
      },
    });
    const reloadTargets = ['quest_template'];
    if (actorEnabled) {
      reloadTargets.push(
        actorPayload?.type === 'gameobject' ? 'gameobject_template' : 'creature_template'
      );
    }
    const reloads = await reloadTablesForRealm(realm, reloadTargets).catch(() => []);
    return res.json({
      ok: true,
      realm,
      quest: { entry: questNewEntry },
      actor: actorEnabled ? actorResult : null,
      reloads,
    });
  } catch (err) {
    console.error('Quest clone failed', err);
    if (err instanceof PortalHttpError) {
      return res.status(err.statusCode).json({ error: err.message });
    }
    return res.status(500).json({ error: 'Unable to clone quest' });
  }
});

app.get('/api/gm/actors/search', requireSession, requireGmRealmQuery, async (req, res) => {
  const realm = req.gmRealm || normalizeRealmInput(req.query?.realm, 'retail') || 'retail';
  const poolRef = getWorldPoolForRealm(realm);
  if (!poolRef) {
    return res.status(503).json({ error: 'World database unavailable' });
  }
  const type = req.query?.type === 'gameobject' ? 'gameobject' : 'creature';
  const table = type === 'gameobject' ? 'gameobject_template' : 'creature_template';
  const rawQuery = String(req.query?.q || '').trim();
  if (!rawQuery) {
    return res.status(400).json({ error: 'Search query required' });
  }
  ...
  const params = [];
  if (numericEntry != null) {
    whereParts.push('(entry = ? OR name LIKE ? ESCAPE \'\\\\\')');
    params.push(numericEntry, buildLikePattern(rawQuery));
  } else {
  ...
  const columns =
    type === 'gameobject'
      ? 'entry, name, type, displayId'
      : 'entry, name, subname, type, faction, npcflag';
  const sql = `
    SELECT ${columns}
    FROM \`${table}\`
    WHERE ${whereParts.join(' AND ')}
  ...
  try {
    ...
    console.error('Actor search failed', err);
    return res.status(500).json({ error: 'Unable to search actors' });
  }
});

app.get('/api/gm/actors/:type/:entry', requireSession, requireGmRealmQuery, async (req, res) => {
  const realm = req.gmRealm || normalizeRealmInput(req.query?.realm, 'retail') || 'retail';
  const poolRef = getWorldPoolForRealm(realm);
  if (!poolRef) {
    return res.status(503).json({ error: 'World database unavailable' });
  }
  const type = req.params?.type === 'gameobject' ? 'gameobject' : 'creature';
  const table = type === 'gameobject' ? 'gameobject_template' : 'creature_template';
  const entry = toSafeNumber(req.params?.entry);
  if (!entry) {
    return res.status(400).json({ error: 'Invalid actor entry' });
  }
  try {
    const template = await fetchSingleRow(poolRef, table, 'entry', entry);
    if (!template) {
      return res.status(404).json({ error: 'Actor template not found' });
    }
    const [rows] = await poolRef.execute(`SELECT MAX(entry) AS maxEntry FROM \`${table}\``);
    const suggestedEntry = Math.max((toSafeNumber(rows?.[0]?.maxEntry) || 900000) + 1, 900000);
    return res.json({
      ok: true,
      realm,
    ...
    console.error('Actor detail lookup failed', err);
    return res.status(500).json({ error: 'Unable to load actor template' });
  }
});
